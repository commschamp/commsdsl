...
std::string LatexCmake::latexSectionPdf() const
{
    ...
    const std::string Templ =
        ...
        "        COMMAND ${CMAKE_COMMAND} -E env TEXINPUTS=\"${CMAKE_CURRENT_SOURCE_DIR}:\" -- ${PDFLATEX_EXE} -file-line-error -output-directory ${CMAKE_CURRENT_BINARY_DIR}/pdf ${MAIN_FILE}\n"
        "        COMMAND ${CMAKE_COMMAND} -E make_directory ${OUTPUT_DIR}/pdf\n"
        "        COMMAND ${CMAKE_COMMAND} -E copy ${CMAKE_CURRENT_BINARY_DIR}/pdf/#^#FILE_BASE#$#.pdf ${OUTPUT_DIR}/pdf/\n"
...
std::string LatexCmake::latexSectionHtml() const
{
    ...
    const std::string Templ =
        ...
        "        COMMAND ${CMAKE_COMMAND} -E env TEXINPUTS=\"${CMAKE_CURRENT_SOURCE_DIR}:\" -- ${HTLATEX_EXE} ${MAIN_FILE} \"${CFG_FILE}\"\n"
        "        COMMAND ${CMAKE_COMMAND} -E make_directory ${OUTPUT_DIR}/html\n"
        "        COMMAND ${CMAKE_COMMAND} -E copy ${CMAKE_CURRENT_BINARY_DIR}/html/#^#FILE_BASE#$#.html ${OUTPUT_DIR}/html/\n"
        "        COMMAND ${CMAKE_COMMAND} -E copy ${CMAKE_CURRENT_BINARY_DIR}/html/#^#FILE_BASE#$#.css ${OUTPUT_DIR}/html/\n"
        "        #^#CMD_APPEND#$#\n"
