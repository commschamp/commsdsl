...
const std::string MinCommsVersionStr("3, 4, 1");
...
std::string Generator::getMessageIdStr(const std::string& externalRef, std::uintmax_t id) const
{
    ...
    assert(m_messageIdField->kind() == commsdsl::parse::Field::Kind::Enum);
...
{
    for (auto& f : files) {
        m_logger.log(commsdsl::parse::ErrorLevel_Info, "Parsing " + f);
        if (!m_protocol.parse(f)) {
            return false;
        }

        if (m_logger.hadWarning()) {
            m_logger.log(commsdsl::parse::ErrorLevel_Error, "Warning treated as error");
            return false;
        }
    }
...
Namespace& Generator::findOrCreateDefaultNamespace()
{
    ...
    auto nsPtr = createNamespace(*this, commsdsl::parse::Namespace(nullptr));
...
const std::string& Generator::fileGeneratedComment()
{
    static const std::string Str =
        "// Generated by commsdsl2comms v" + std::to_string(commsdsl::parse::versionMajor()) +
        '.' + std::to_string(commsdsl::parse::versionMinor()) + '.' +
        std::to_string(commsdsl::parse::versionPatch()) + '\n';
    return Str;
}
        ...
        for (auto& m : info.m_dslNames) {
            ...
            auto iter =
                std::lower_bound(
                    ...
                    [](commsdsl::parse::Message msg, const std::string& mParam)
                    {
