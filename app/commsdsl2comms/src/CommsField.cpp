...
bool CommsField::commsPrepare()
{
    ...
    auto& generator = m_genField.genGenerator();
    auto inputCodePrefix = comms::genInputCodeRelPathFor(m_genField, generator);
    CommsCustomCode customCodeTmp;
    customCodeTmp.m_constructBody = generator.genReadCodeInjectCode(inputCodePrefix + strings::genConstructBodyFileSuffixStr(), "Replace default constructor body only", &customCodeTmp.m_hasConstructBody);
    ...
    commsAssignCustomCode(customCodeTmp.m_constructBody, customCodeTmp.m_hasConstructBody, m_customCode.m_constructBody, m_customCode.m_hasConstructBody);
    commsAppendCustomCode(customCodeTmp.m_inc, customCodeTmp.m_hasInc, m_customCode.m_inc, m_customCode.m_hasInc);
    commsAppendCustomCode(customCodeTmp.m_public, customCodeTmp.m_hasPublic, m_customCode.m_public, m_customCode.m_hasPublic);
    commsAppendCustomCode(customCodeTmp.m_protected, customCodeTmp.m_hasProtected, m_customCode.m_protected, m_customCode.m_hasProtected);
    commsAppendCustomCode(customCodeTmp.m_private, customCodeTmp.m_hasPrivate, m_customCode.m_private, m_customCode.m_hasPrivate);
    commsAppendCustomCode(customCodeTmp.m_append, customCodeTmp.m_hasAppend, m_customCode.m_append, m_customCode.m_hasAppend);
...
std::string CommsField::commsCommonCode() const
{
    ...
    if ((!base.empty()) && (body.empty())) {
        ...
        util::GenReplacementMap repl = {
            {"NAME", comms::genClassName(m_genField.genName())},
            {"BASE", std::move(base)},
        };
    ...
    util::GenReplacementMap repl = {
        ...
        {"SCOPE", comms::genScopeFor(m_genField, generator)},
        {"DEF", std::move(def)},
...
{
    if (m_genField.genParseObj().parseSemanticType() == commsdsl::parse::ParseField::ParseSemanticType::Length) {
...
CommsField::CommsIncludesList CommsField::commsDefIncludes() const
{
    ...
    if (comms::genIsGlobalField(m_genField)) {
        auto relHeader = comms::genRelCommonHeaderPathFor(m_genField, generator);
...
{
    if (!m_genField.genGenerator().genSchemaOf(m_genField).genVersionDependentCode()) {
        return false;
    }
...
{
    if ((m_customCode.m_hasLength) || (!commsDefLengthFuncBodyImpl().empty())) {
        return true;
    }
...
{
    auto& generator = static_cast<CommsGenerator&>(m_genField.genGenerator());
    auto level = generator.commsGetCustomizationLevel();
    if (level == CommsGenerator::CommsCustomizationLevel::Full) {
        return true;
    }

    if (m_genField.genParseObj().parseIsCustomizable()) {
        return true;
    }
...
void CommsField::commsAssignCustomCode(
    const std::string& code,
    bool hasCode,
    std::string& to,
    bool& toHasCode)
{
    if ((!hasCode) && (toHasCode)) {
        return;
    }
...
{
    return m_genField.genParseObj().parseMinLength();
}

std::size_t CommsField::commsMaxLengthImpl() const
{
    return m_genField.genParseObj().parseMaxLength();
}
...
{
    auto& generator = m_genField.genGenerator();
    auto customNamePath = comms::genInputCodeAbsPathFor(m_genField, generator) + strings::genNameFileSuffixStr();
    ...
    util::GenReplacementMap repl = {
        {"SCOPE", comms::genScopeFor(m_genField, generator)},
        {"NAME", util::genDisplayName(m_genField.genParseObj().parseDisplayName(), m_genField.genParseObj().parseName())},
...
{
    auto& schema = commsdsl::gen::GenGenerator::genSchemaOf(m_genField);
...
void CommsField::commsAddFieldDefOptions(commsdsl::gen::util::GenStringsList& opts, bool tempFieldObj) const
{
    ...
    if (commsIsFieldCustomizable()) {
        auto& gen = static_cast<const CommsGenerator&>(m_genField.genGenerator());
        opts.push_back("typename TOpt::" + comms::genScopeFor(m_genField, m_genField.genGenerator(), gen.commsHasMainNamespaceInOptions(), true));
    }
    ...
    if (m_customCode.m_hasWrite) {
        util::genAddToStrList("comms::option::def::HasCustomWrite", opts);
    }

    if (m_forcedPseudo || m_genField.genParseObj().parseIsPseudo()) {
...
{
    return m_extraCustomCode.m_hasExtend;
}
...
bool CommsField::commsCopyCodeFromInternal()
{
    ...
    gen.genLogger().genDebug(
        "Copying code: " + commsField->commsGenField().genName() + " -> " + commsGenField().genName());
    ...
    bool& hasCode)
{
    ...
    do {
        ...
        commsAssignCustomCode(customCode, hasCodeTmp, code, hasCode);
    } while (false);
...
std::string CommsField::commsCustomValueCodeInternal(bool& hasRealCode) const
{
    auto& generator = m_genField.genGenerator();
    auto relPath = comms::genInputCodeRelPathFor(m_genField, generator);

    return generator.genReadCodeInjectCode(relPath + strings::genValueFileSuffixStr(), "Override value() and related function(s)", &hasRealCode);
}

std::string CommsField::commsCustomReadCodeInternal(bool& hasRealCode) const
{
    auto& generator = m_genField.genGenerator();
    auto relPath = comms::genInputCodeRelPathFor(m_genField, generator);
    ...
    bool hasBody = false;
    auto code = generator.genReadCodeInjectCode(relPath + strings::genReadFileSuffixStr(), "Override full read() function", &hasFullFunc);
    std::string bodyCode;
    if (!hasFullFunc) {
        bodyCode = generator.genReadCodeInjectCode(relPath + strings::genReadBodyFileSuffixStr(), "Override body only of read() function", &hasBody);
    }
...
std::string CommsField::commsCustomWriteCodeInternal(bool& hasRealCode) const
{
    auto& generator = m_genField.genGenerator();
    auto relPath = comms::genInputCodeRelPathFor(m_genField, generator);
    ...
    bool hasFullFunc = false;
    bool hasBody = false;
    auto code = generator.genReadCodeInjectCode(relPath + strings::genWriteFileSuffixStr(), "Override full write() function", &hasFullFunc);
    std::string bodyCode;
    if (!hasFullFunc) {
        bodyCode = generator.genReadCodeInjectCode(relPath + strings::genWriteBodyFileSuffixStr(), "Override body only of write() function", &hasBody);
    }
...
std::string CommsField::commsCustomRefreshCodeInternal(bool& hasRealCode) const
{
    auto& generator = m_genField.genGenerator();
    auto relPath = comms::genInputCodeRelPathFor(m_genField, generator);
    ...
    bool hasFullFunc = false;
    bool hasBody = false;
    auto code = generator.genReadCodeInjectCode(relPath + strings::genRefreshFileSuffixStr(), "Override full refresh() function", &hasFullFunc);
    std::string bodyCode;
    if (!hasFullFunc) {
        bodyCode = generator.genReadCodeInjectCode(relPath + strings::genRefreshBodyFileSuffixStr(), "Override body only of refresh() function", &hasBody);
    }
...
std::string CommsField::commsCustomLengthCodeInternal(bool& hasRealCode) const
{
    auto& generator = m_genField.genGenerator();
    auto relPath = comms::genInputCodeRelPathFor(m_genField, generator);
    ...
    bool hasFullFunc = false;
    bool hasBody = false;
    auto code = generator.genReadCodeInjectCode(relPath + strings::genLengthFileSuffixStr(), "Override full length() function", &hasFullFunc);
    std::string bodyCode;
    if (!hasFullFunc) {
        bodyCode = generator.genReadCodeInjectCode(relPath + strings::genLengthBodyFileSuffixStr(), "Override body only of length() function", &hasBody);
    }
...
std::string CommsField::commsCustomValidCodeInternal(bool& hasRealCode) const
{
    auto& generator = m_genField.genGenerator();
    auto relPath = comms::genInputCodeRelPathFor(m_genField, generator);
    ...
    bool hasFullFunc = false;
    bool hasBody = false;
    auto code = generator.genReadCodeInjectCode(relPath + strings::genValidFileSuffixStr(), "Override full valid() function", &hasFullFunc);
    std::string bodyCode;
    if (!hasFullFunc) {
        bodyCode = generator.genReadCodeInjectCode(relPath + strings::genValidBodyFileSuffixStr(), "Override body only of valid() function", &hasBody);
    }
...
std::string CommsField::commsCustomNameCodeInternal(bool& hasRealCode) const
{
    auto& generator = m_genField.genGenerator();
    auto relPath = comms::genInputCodeRelPathFor(m_genField, generator);
    ...
    bool hasFullFunc = false;
    bool hasBody = false;
    auto code = generator.genReadCodeInjectCode(relPath + strings::genNameFileSuffixStr(), "Override full name() function", &hasFullFunc);
    std::string bodyCode;
    if (!hasFullFunc) {
        bodyCode = generator.genReadCodeInjectCode(relPath + strings::genNameBodyFileSuffixStr(), "Override body only of name() function", &hasBody);
    }
...
{
    auto& generator = m_genField.genGenerator();
    auto filePath = commsdsl::gen::comms::genCommonHeaderPathFor(m_genField, generator);
    ...
    util::GenReplacementMap repl = {
        ...
        {"FIELD_SCOPE", comms::genScopeFor(m_genField, generator)},
        {"INCLUDES", util::genStrListToString(includes, "\n", "\n")},
        {"NS_BEGIN", comms::genNamespaceBeginFor(m_genField, generator)},
        {"NS_END", comms::genNamespaceEndFor(m_genField, generator)},
        {"DEF", commsCommonCode()},
    };
...
{
    auto& generator = m_genField.genGenerator();
    auto filePath = commsdsl::gen::comms::genHeaderPathFor(m_genField, generator);
    ...
    util::GenReplacementMap repl = {
        {"GENERATED", CommsGenerator::commsFileGeneratedComment()},
        {"FIELD_NAME", util::genDisplayName(m_genField.genParseObj().parseDisplayName(), m_genField.genParseObj().parseName())},
        {"INCLUDES", util::genStrListToString(includes, "\n", "\n")},
...
std::string CommsField::commsFieldDefCodeInternal() const
{
    ...
    if (pub.empty() && prot.empty() && priv.empty() && (!m_extraCustomCode.m_hasExtend)) {
...
{
    if (commsIsVersionOptional()) {
        return "/// @brief Inner field of @ref " + comms::genClassName(m_genField.genName()) + " optional.";
    }
...
std::string CommsField::commsDocDetailsInternal() const
{
    std::string result;
    do {
        auto& desc = m_genField.genParseObj().parseDescription();
...
{
    std::string result;
    auto deprecatedVersion = m_genField.genParseObj().parseDeprecatedSince();
    auto& generator = m_genField.genGenerator();
...
std::string CommsField::commsTemplateParamsInternal() const
{
    ...
    if (comms::genIsGlobalField(m_genField)) {
        ...
        result += comms::genScopeForOptions(strings::genDefaultOptionsStr(), m_genField.genGenerator());
...
std::string CommsField::commsDefConstructPublicCodeInternal() const
{
    ...
    util::GenReplacementMap repl = {
        {"CLASS_NAME", comms::genClassName(m_genField.genParseObj().parseName())},
    };
    ...
    if (m_customCode.m_hasConstructBody) {
        repl["BODY"] = m_customCode.m_constructBody;
        repl["CUSTOM"] = m_extraCustomCode.m_construct;
        return util::genProcessTemplate(Templ, repl);
    }
    ...
    repl["CUSTOM"] = m_extraCustomCode.m_construct + m_customCode.m_constructBody;

    if (body.empty()) {
        static const std::string DefaultTempl =
            "/// @brief Default constructor.\n"
            "#^#CLASS_NAME#$##^#SUFFIX#$##^#ORIG#$#() = default;\n"
            "#^#CUSTOM#$#\n";
        return util::genProcessTemplate(DefaultTempl, repl);
    }

    repl["BODY"] = std::move(body);
    return util::genProcessTemplate(Templ, repl);
}
...
std::string CommsField::commsDefNameFuncCodeInternal() const
{
    ...
    std::string custom;
    auto overrideType = m_genField.genParseObj().parseNameOverride();
    if (m_customCode.m_name.empty() && (!commsDefHasNameFuncImpl())) {
        return strings::genEmptyString();
    }

    if (!commsHasOrigCode(overrideType)) {
        return m_customCode.m_name;
    }
    ...
    util::GenReplacementMap repl = {
        {"SCOPE", comms::genCommonScopeFor(m_genField, generator)},
        {"CUSTOM", m_customCode.m_name},
    };
...
std::string CommsField::commsDefReadFuncCodeInternal() const
{
    ...
    util::GenReplacementMap repl;
    std::string body;
    if (commsHasOrigCode(m_genField.genParseObj().parseReadOverride())) {
...
std::string CommsField::commsDefWriteFuncCodeInternal() const
{
    ...
    if (commsHasOrigCode(m_genField.genParseObj().parseWriteOverride())) {
...
std::string CommsField::commsDefRefreshFuncCodeInternal() const
{
    ...
    util::GenReplacementMap repl;
    std::string body;
    if (commsHasOrigCode(m_genField.genParseObj().parseRefreshOverride())) {
...
std::string CommsField::commsDefLengthFuncCodeInternal() const
{
    ...
    std::string body;
    if (commsHasOrigCode(m_genField.genParseObj().parseLengthOverride())) {
...
std::string CommsField::commsDefValidFuncCodeInternal() const
{
    ...
    util::GenReplacementMap repl;
    std::string body;
    if (commsHasOrigCode(m_genField.genParseObj().parseValidOverride())) {
...
std::string CommsField::commsDefMembersCodeInternal() const
{
    ...
    util::GenReplacementMap repl = {
        {"CLASS_NAME", comms::genClassName(m_genField.genName())},
        {"BODY", std::move(body)}
    };
    ...
    if (comms::genIsGlobalField(m_genField)) {
        auto prefix =
            ...
            "template <typename TOpt = " + comms::genScopeForOptions(strings::genDefaultOptionsStr(), m_genField.genGenerator()) + ">";
...
std::string CommsField::commsCustomizationOptionsInternal(
    ...
    bool hasBase) const
{
    if ((!m_genField.genIsReferenced()) && (comms::genIsGlobalField(m_genField))) {
        return strings::genEmptyString();
    }
    ...
    if (!membersBody.empty()) {
        ...
        util::GenReplacementMap repl = {
            {"NAME", comms::genClassName(m_genField.genParseObj().parseName())},
            {"SUFFIX", strings::genMembersSuffixStr()},
            {"BODY", std::move(membersBody)},
        };

        if (hasBase) {
            auto& commsGen = static_cast<const CommsGenerator&>(m_genField.genGenerator());
            bool hasMainNs = commsGen.commsHasMainNamespaceInOptions();
            repl["EXT"] = " : public TBase::" + comms::genScopeFor(m_genField, m_genField.genGenerator(), hasMainNs) + strings::genMembersSuffixStr();
        }
    ...
    do {
        ...
        if ((!extraOpts.empty()) && (hasBase)) {
            auto& commsGen = static_cast<const CommsGenerator&>(m_genField.genGenerator());
            bool hasMainNs = commsGen.commsHasMainNamespaceInOptions();
            extraOpts.push_back("typename TBase::" + comms::genScopeFor(m_genField, m_genField.genGenerator(), hasMainNs));
        }

        auto docStr =
            "/// @brief Extra options for @ref " +
            comms::genScopeFor(m_genField, m_genField.genGenerator()) + " field.";
