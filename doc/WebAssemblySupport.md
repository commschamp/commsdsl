...
- The relevant include directories can be brought it using **OPT_EXTRA_INCLUDE_DIRS** cmake variable.
...
```js
var msg1 = new my_prot.message_Msg1();
var msg1Name = msg1.name(); // returns std::string by value, not deletion is required
...
The emscripten binding code (generated by the **commsdsl2comms**) names the enum by appending `_ValueType` suffix to
...
functions for the relevant field class(es). The **commsdsl2emscripten** coge generator provides
