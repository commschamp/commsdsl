...
However, the definition and the use of the enum values may differ. For example:
```xml
<shema name="my_prot">
    <fields>
...
allowing externding it and overriding relevant member functions in the target language.
...
Then the target language can use the required extra functions:
```python
m = my_prot.message_Msg1()
